...
function Golfer({golfers, classifications, change_counter}) {
  ...
  const [statement, setStatement] = useState("temporary statement");
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:9292/golfer_statement/${the_golfer.id}`)
      .then((r) => r.json())
      .then((statement) => setStatement(statement));
  ...
  return (
    <>
      ...
      <p id="the_statement">{statement}</p>
    </>
  )
}
